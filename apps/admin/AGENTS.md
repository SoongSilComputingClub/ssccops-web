...
- **학술 어휘는 한 벌이다**(#592 · ssccops#439 어휘 표 · ADR-0043). 스터디·프로젝트·트랙 묶음은 «학술 프로그램», 그 하나는 «프로그램»(«프로그램 목록»·«내 프로그램»·«없는 프로그램입니다»), 권한 이름은 «학술 프로그램 관리(ACADEMIC_PROGRAM_MANAGE)»다. «학술 활동»·«스터디·프로젝트»·«활동 목록»은 쓰지 않는다 — 유형을 나열할 때만 «스터디·프로젝트·트랙». «회차»·«출석»·«활동일»(콘텐츠의 `actvYmd`)은 프로그램 안의 일이라 그대로다. 주소·식별자(`/academic-programs`·`acdm_actv`·`academicProgram`)는 바꾸지 않는다 — 문구만이다.
...
- **검색엔진에는 전부 닫혀 있다**(#602 · ssccops#444) — `app/robots.ts`가 `Disallow: /`, 루트 `metadata.robots`가 noindex. 그전에는 robots.txt가 404라 로그인·OAuth 동의 화면이 색인될 수 있었다. 검색으로 올 사람은 www가 받는다. 매처가 `.txt`를 빼므로 미들웨어가 robots.txt를 로그인으로 밀지 않는다.
