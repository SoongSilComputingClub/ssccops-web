...
- **시스템 폼(`sysYn`)은 폼 목록에서 빠지고 `/forms/system`(`views/system-form-list`)이 싼다**(#553 · ssccops#415). 응답을 LMS에서 받는 폼이라 상세는 «공개 링크» 카드 대신 «응답 받는 곳»을 그리고(`publicUrl`도 `null`), 기획안(`PROPOSAL`)이면 `NEXT_PUBLIC_LMS_ORIGIN`(`shared/config/lms-routes.ts`)이 있을 때만 LMS 제출 화면 버튼을 둔다 — 비면 문구만. 판정은 서버의 `sysYn`이지 라벨·제목이 아니고, 서버 필터 파라미터는 더하지 않는다(`GET /v1/forms` 전량을 화면이 가른다). 시스템 폼 카드에는 복제·삭제가 없다.
- **시스템 폼(`sysYn`)의 문항 구조는 편집기가 잠근다**(#554 · ssccops#416 · 서버 #498 → 좁힘 #563 · ssccops#421 · 서버 #505). `QitemComposer`의 `questionsLocked`가 문항 추가·삭제·이동·구조 속성(유형·필수·선택지·분기·형식 검증·최대 선택 수)과 페이지 추가·삭제·순서를 잠그고(문항의 `pageSeq`가 페이지 index라 페이지 구조가 곧 문항이다), 펼친 카드(`LockedQitemBody`)는 **질문 문구·문항 설명 두 칸만 입력란**이고 나머지는 값만 그린다 — 이관이 읽는 것은 답이지 문구가 아니라 운영진이 학기마다 안내를 다듬는 자리를 막지 않는다. 제목·페이지 설명·접수 기간·라벨·다중 응답도 열려 있다. 서버는 구조 속성이 바뀐 저장만 409 `SYSTEM_FORM_QUESTIONS_LOCKED`로 거절하므로 설명만 고친 자동 저장(구성을 그대로 되보내는 전체 PATCH)은 통과하고, 409는 저장 상태 표시줄이 «저장 실패 — 시스템 폼의 문항은 잠겨 있습니다»로 받는다. `systemRequiredQitemIds`의 삭제 잠금은 이 안에 들어가지만 서버가 아직 내리므로 코드 경로는 남긴다.
...
- **학술 프로그램은 분류가 아니라 응답의 `academicProgram`으로 가른다**(#587 · ssccops#435 · ADR-0043 · 서버 #519). 행사 목록 필터 줄의 셋째 축 «전체 | 행사 | 학술 프로그램»은 `?kind=event|program`이고 **화면이 거른다**(서버 파라미터 없음 · 보관 제외와 같은 이유), 카드·표에는 유형(`typeNm` — 스터디·프로젝트·트랙, 어휘는 `acdm_actv_type`) `Pill`이 분류 옆에 선다. 프로그램 행사의 수정 화면은 분류 셀렉트를 잠근다(`EventForm` · 사유 `title` = `CLASSIFICATION_LOCKED_FOR_PROGRAM`) — 값이 뜻이 없어서이고, 잠금을 우회한 저장은 서버가 409 `EVENT_CLASSIFICATION_LOCKED_FOR_PROGRAM`로 거절해 같은 문장으로 뜬다(값이 바뀔 때만 — 상세를 그대로 되보내는 저장은 통과). 분류로 «학술» 칩을 만들지 않는 이유는 이관이 `EVENT`로 고정한 뒤 운영진이 바꿔 dev에서 이미 갈렸기 때문이다.
