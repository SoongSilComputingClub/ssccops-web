        ...
        return (
          <span className="flex items-center gap-[10px]">
            {/*
              ...
              `S6847`·`S1082`(비대화 요소 핸들러 · 키보드 없음)는 여기서 오탐이다 (#658).
              태그는 이미 시맨틱하고(label + input[type=checkbox]) 이 `onClick`은 **동작이
              아니라 차단**이다 — 없는 마우스 동작을 키보드로 흉내 낼 것이 없다. 핸들러를
              안의 input 으로 옮기면 규칙은 조용해지지만 여백(16px 밖 히트 영역)을 누른 클릭이
              행까지 올라가 회원 상세로 넘어간다 — 규칙을 위해 32px 히트 영역을 버리는 셈이다.
            */}
            <label
