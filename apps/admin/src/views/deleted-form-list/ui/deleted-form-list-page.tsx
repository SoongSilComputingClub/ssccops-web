...
export function DeletedFormListPage() {
   ...
   *
   * `deleted=true`는 아직 확정되지 않은 파라미터라(ssccops-server#329), 이 값을 모르는 배포는
   * 그것을 조용히 무시하고 **살아 있는 폼 전부**를 내려준다. 그대로 그리면 이 화면은 멀쩡한
   * 폼들을 "삭제됨" 배지와 함께 세워 놓고 되살리라고 권한다 — 운영진에게는 자기 폼이 전부
   * 지워진 것으로 보인다.
   *
   * `delDt`가 없는 항목을 버리면 그 배포에서는 목록이 빈다. 그것이 맞는 결과다 — 삭제 경로가
   * 없는 서버에는 지워진 폼도 없다.
   */
