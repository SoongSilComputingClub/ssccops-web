...
export interface EventImageTicket {
  /** 웹이 한 번 PUT 할 서명 주소 */
  uploadUrl: string;
  /** 본문·대표 이미지에 박아 넣을 공개 읽기 주소 */
  publicUrl: string;
  /** 버킷 키 — 운영이 파일을 지목하는 수단이다 */
  objectKey: string;
  /** 서명 유효 시간(초) — 재시도가 가능한지 판단하는 근거다 */
  expiresInSeconds: number;
}
 ...
 * 첨부를 잠그는 이유가 이것이다.
 *
 * 응답의 `publicUrl`을 그대로 쓴다. 공개 도메인은 환경(dev·prod)마다 다르고 서버 설정
 * (`DEV_R2_PUBLIC_BASE_URL`·`PROD_R2_PUBLIC_BASE_URL`)에만 있다 — 웹이 `objectKey`로 주소를
 * 조립하면 도메인 사전이 두 벌이 되어 한쪽만 바뀌는 날 본문의 이미지가 통째로 깨진다.
 */
