 ...
 * 잠근 채 사유를 밝힌다** — 이미 이 화면을 보고 있는 사람에게서 버튼만 사라지면 기능이
 * 없어진 것인지 권한 문제인지 알 수 없다.
 *
 * **사유는 `title`에만 두지 않는다** (#204). title 은 마우스를 올려야 보이고 터치에서는
 * 아예 보이지 않아, 잠긴 이유를 읽을 방법이 사실상 없는 사람이 생긴다. 그래서 셋은 패널
 * 머리말에, 승인만 막는 사유(`acceptBlockReason`)는 그 아래 배너에, 의견 미입력은 입력란
 * 바로 아래에 각각 글로 선다 — 자리는 다르지만 규칙은 하나다. title 은 거들 뿐이다.
 *
 * ── 성공 후 갱신은 재조회다 ────────────────────────────────────
    ...
    <Card>
      ...
      */}
      ...
      <Field className="mt-4" label="검토 의견 (수정요청 · 반려 시 필수)">
        ...
        />
        {/*
          의견이 비어 수정요청·반려가 잠겨 있으면 그 사실을 여기서 말한다 (#204).

          버튼의 title 로도 붙지만 그것은 마우스를 올려야 보이고 터치에서는 아예 보이지
          않는다 — 바로 위 acceptBlockReason 배너가 같은 이유로 본문에 서 있고, 같은 근거가
          이쪽에도 그대로 적용된다. 특히 기획안 검토에서는 파싱 실패 배너가 "이 사유를 검토
          의견에 적어 수정요청으로 돌려주세요"라고 **행동을 지시하는데** 그 버튼이 잠겨 있어,
          사유가 안 보이면 시키는 대로 하려다 막힌 것으로 읽힌다.

          배너가 아니라 이 자리인 것은, 잠금을 푸는 방법이 곧 **바로 위 입력란을 채우는 것**
          이기 때문이다. 사유와 그것을 푸는 곳이 붙어 있어야 다음 행동이 한 번에 읽힌다.
          패널 전체가 잠겼으면(권한·제출 전·이미 처리) 의견을 적어도 풀리지 않으므로 그때는
          기본 도움말로 둔다 — 고칠 수 없는 것을 고치라고 말하지 않는다.
        */}
        {!lockReason && !opinionFilled ? (
          <div className="mt-[5px] text-[12.5px] leading-[1.7] text-amber">
            {OPINION_REQUIRED}
          </div>
        ) : (
