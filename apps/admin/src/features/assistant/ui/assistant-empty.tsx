...
export function AssistantEmpty({
   ...
   * 추천 질문이 빈 배열인 것만으로는 «코퍼스가 비었다»와 «문서는 있는데 답변에 쓰이는 것이
        ...
        <div className="flex flex-col gap-[7px]">
          ...
          {/*
            **서버가 내린 추천 질문이다**(§13.3). 웹에 하드코딩하면 업로드 다음 날부터 거짓말을
            한다 — 후보가 «어느 문서가 있어야 답할 수 있는가»로 묶여 있어 그 문서가 쓰이는 중이
            아니면 내려오지 않는다.
          */}
 ...
 * 서버가 그 한 값에 «색인 중»·«색인 실패»·«제외된 문서뿐»·«쓰이지만 재색인 중»을 모두
 ...
 *
 * ── 링크가 «아직 쓰이지 않는 문서»로 데려간다 ──────────────────
 * `NONE_EFFECTIVE`의 링크는 적용 상태 필터를 실어 보낸다 — 문서가 수십 건일 때 «답변에
 * 사용을 누르세요»라고 말하면서 걸러지지 않은 전체 목록으로 데려가면 그 말이 다시 헛돈다. **한
 * «행»을 가리킬 수는 없다**: 패널이 받는 것은 상태 하나뿐이고(`suggestions`에 문서 식별자가
 * 없다) 아직 쓰이지 않는 문서가 여러 건이면 «그 행»이 하나로 정해지지도 않는다.
 *
 * **이 문구는 RAG 설정 화면의 버튼 이름과 한 쌍이다**(#468). 여기서 «시행»이라 부르면서
 * 데려간 자리에 그 이름의 버튼이 없으면, 안내를 읽고 따라간 사람이 무엇을 눌러야 할지 모른다.
 */
  ...
  return (
    <div className="rounded-xl border border-line bg-subtle px-[13px] py-[11px] text-[13px] leading-[1.7] text-n400">
      {corpusState === "NONE_EFFECTIVE" ? (
        canManage ? (
          <>
            올린 문서가 아직 답변에 쓰이지 않습니다 — <RagSettingsLink applyStatus="DRAFT" />
            에서 «답변에 사용»을 눌러야 답변에 쓰입니다
          </>
        ) : (
          "아직 답변에 쓰이는 규정 문서가 없습니다 — 운영진에게 문의해주세요"
        )
      ) : canManage ? (
        <>
