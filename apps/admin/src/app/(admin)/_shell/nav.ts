...
export interface NavItem {
   ...
   * 항목이 빠진 뒤 **보이는 항목 기준**으로 찍는다(빈 구분 제목 없음). «설정» 묶음이 화면 10개를
   * 한 줄로 이어 무엇의 설정인지 갈리지 않아 뒀다 — 운영 · 회원 · 폼 · 행사 · 규정 도우미 ·
   * 통합(#649) 여섯이고, 다른 묶음은 비워 둔다. 클릭되지 않는다.
   */
  section?: string;
}
...
export const NAV_GROUPS: NavGroup[] = [
  ...
  },
  {
    ...
    items: [
       ...
       *
       * **«통합» 구분에 둔다** (#649 · ssccops#469). #639가 «운영»에 둔 근거는 «만지는 사람이
       * 시스템 운영자 한 종류»였는데 그것은 권한의 이야기(`SUPER`)이고, 구분 제목이 답하는
       * 것은 «무엇에 대한 설정인가»다 — 그 기준이라면 권한 관리도 «운영»에 가야 한다. 이 표가
       * 정하는 것은 **세 앱(admin·lms·www) 중 어디에 알림이 보이는가**여서 도메인 축이 없고,
       * 담는 내용도 운영 사건 5종과 회원 사건 6종을 함께 갖는다.
       *
       * 맨 아래인 것은 도메인 기준정보 뒤에 시스템 정책이 오는 설정 화면의 통례이고, 목록에서
       * `SUPER`만 보는 유일한 자리라 위에 두면 대부분의 운영자에게 첫 구분 제목이 사라진
       * 자리처럼 보인다. 세 앱에 걸친 설정이 늘면(푸시 · 앱 간 연동) 같은 자리에 모인다.
       */
      {
        section: "통합",
        label: "알림 유형",
