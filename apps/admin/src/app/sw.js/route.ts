...
/*
 * `GET /sw.js` — 서비스워커 (#604 · ADR-0045 · #671).
 *
 ...
 *
 * `apiBaseUrl`은 `apiFetch`와 같은 값을 읽으므로 새 env가 없다 — 패키지가 오리진만 뽑는다. 값이 비면
 * API 응답은 캐시하지 않는다(요청도 어차피 나가지 않는다 · `CLIENT_CONFIG_MISSING`).
 */
