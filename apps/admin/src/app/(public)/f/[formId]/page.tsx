 ...
 * `GET /v1/forms/{formId}/public`은 인증을 요구하므로(ADR-0010 — 응답자는 전원 회원이다),
 * 서버가 제목·안내 문구만 내주는 `/public/v1/forms/{formId}/meta`를 따로 열었다
 * (ssccops-server#247).
 *
 * **접수 상태·마감일은 담지 않는다.** 메신저는 OG를 한 번 캐싱하면 갱신하지 않아 카드가
 * 굳는다 — 담으면 마감된 뒤에도 "모집 중"이라 말하는 카드가 방에 남는다(ssccops#194 제약 ②).
 * 서버 응답에 애초에 그 값이 없으므로 여기서 실수로 실을 수도 없다.
 *
 * 조회가 실패하거나 아직 접수를 연 적 없는 폼이면 **기본 메타로 조용히 떨어진다** — 여기서
 * 404를 내지 않는 것은 그 판단을 본문 쪽 한 곳에서만 하기 때문이고, 공유 카드가 밋밋한 것과
 * 페이지가 안 뜨는 것은 무게가 다르다(`apps/www` 행사 OG와 같은 자리).
 */
