...
/*
 * `GET /sw.js` — 서비스워커 (#606 · ADR-0045 · 어드민 #604와 같은 모양).
 *
 ...
 *
 * `apiOrigin`은 `NEXT_PUBLIC_API_BASE_URL`의 오리진이다 — `apiFetch`와 같은 값을 읽는다. 값이 비면
 * API 응답은 캐시하지 않는다(요청도 어차피 나가지 않는다 · `CLIENT_CONFIG_MISSING`). `appOrigins`는
