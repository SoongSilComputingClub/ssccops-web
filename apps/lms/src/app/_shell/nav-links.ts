 ...
 *
 * **역할 판정은 이 이슈에서 하지 않는다.** 인증 세션 슬라이스(`entities/session`)와 역할별
 * 필터링은 화면 이슈들이 붙인다 — 지금은 프로토타입이 정한 두 묶음을 상수로 두고, `role`
 * 필드로 나눠 둔다. 로그인 여부로 갈리는 항목(로그아웃 등)은 목차에 없고 `AuthNav`가
