...
/**
 * 로그인 실패를 학술 대시보드로 되돌린다.
 *
 * 어드민은 로그인 화면으로 돌려보내지만 이 앱에는 그런 화면이 없다(apps/www와 같다). 이 앱은
 * 전 화면이 로그인 필수라 어느 화면이든 실패 사유를 설명하고 다시 로그인 버튼을 내주는
 * 공용 게이트를 두르므로, **첫 화면인 `/studio`**로 보낸다. 사유 코드를 그대로 실어 보내는
 * 것은, 하나로 뭉뚱그리면 사용자가 취소한 것인지 코드 교환이 깨진 것인지 로그를 봐도 가릴 수
 * 없기 때문이다.
 */
function failureRedirect(origin: string, error: string) {
  const url = new URL(ROUTES.studio, origin);
...
function resolveNext(request: NextRequest): string {
  ...
  if (fromCookie) {
    ...
    if (decoded) return safeNextPath(decoded, ROUTES.studio);
  }
  return safeNextPath(new URL(request.url).searchParams.get("next"), ROUTES.studio);
}
