 ...
 * 기획안 피처 배럴 (#171).
 *
 * ⚠️ **클라이언트 훅(`useResubmitForm`)은 여기서 재export 하지 않는다** — 이 배럴은 SSR 로더
 * (`loadMyApplications`·`loadProposalDetail`)를 재export 하고, 그 로더는 서버 전용 조회
 * (`entities/response/api/*` → `next/headers`)를 끌어온다. 클라이언트 컴포넌트가 이 배럴을 통해
 * 훅을 가져오면 서버 모듈이 클라 번들로 딸려 들어가 빌드가 깨진다(`features/academic-session`
 * 배럴과 같은 규칙). 훅은 `@/features/proposal/model/use-resubmit-form`에서 직접 임포트한다.
 *
