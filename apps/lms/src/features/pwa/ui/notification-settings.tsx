...
/**
 * «알림 설정» 카드 — `/my`와 `/notifications` 맨 위 (#606 · #634 · ssccops#461).
 *
 * 카드(스위치·상태 문구·«테스트 알림 보내기»)는 `@ssccops/pwa/ui`의 `NotificationSettingsCard` 한 벌이고
 * 이 파일은 lms의 서버 호출(`entities/push` `pushApi` · `entities/notification` `notificationApi.sendTest` —
 * `browser-client`)과 `app: "LMS"`를 꽂는 것이 전부다. #634 전에는 `PushToggleCard`·`usePushToggle`이 여기
 * 사본으로 있었다.
 */
