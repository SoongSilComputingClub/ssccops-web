...
export function submitSessionErrorMessage(error: unknown): string {
  ...
  switch (error.code) {
    ...
    case AUTH_ERROR.UNAUTHORIZED:
      return "로그인이 풀렸습니다 — 다시 로그인한 뒤 제출해주세요";
    default:
      return error.message;
  }
}
...
export function sessionPhotoErrorMessage(error: unknown): string {
  ...
  switch (error.code) {
    case SESSION_PHOTO_ERROR.UNSUPPORTED_IMAGE_TYPE:
      return "회차 기록은 저장됐습니다 — 인증사진은 JPG·PNG로 다시 올려주세요";
    ...
    case API_ERROR.NETWORK_ERROR:
      return "회차 기록은 저장됐습니다 — 인증사진은 올리지 못했으니 출석부에서 다시 올려주세요";
    default:
      return "회차 기록은 저장됐습니다 — 인증사진은 올리지 못했습니다: " + error.message;
  }
}
