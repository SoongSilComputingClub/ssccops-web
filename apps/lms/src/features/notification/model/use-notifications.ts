...
/*
 * `/notifications` — 목록·«더 보기»·읽음 처리·이동 (#606 · ssccops#448 · #665 · #671).
 *
 * 몸통도 배선도 `@ssccops/pwa`다 — 커서 페이징·범위 칩(#643 · ADR-0047)·«배지는 언제나 이 앱 수»
 * 규칙은 세 앱이 같고, 라우터 이동과 401·403 판정의 배선은 www와 같아 `useGatedNotificationList`로
 * 올렸다(#671). 그쪽 주석이 정본이고 여기 남은 것은 이 앱의 값 다섯이다.
 *
 * 이 앱의 `apiFetch`는 401·403을 리다이렉트하지 않고 오류로 올린다 — 그래서 상태에 `unauthenticated`·
 * `signup-required`가 따로 있고 화면이 로그인 게이트·가입 안내를 그린다. 판정은 이 앱의 `ApiError`를
 * 아는 `shared/api/auth-error.ts`가 하고 패키지는 그 둘을 받는다. «모두 읽음» 실패는 전역 토스트가
 * 없어(`features/share` 주석) 훅의 `actionError` 한 줄로 목록 위에 보인다.
 *
