 ...
 *
 * ── 왜 복사인가 — 근거는 ADR-0052다 (2026-09-26 정정) ──────
 * 앱끼리 소스를 공유하지 않는다(AGENTS.md). 그 위에 이 편집기를 `packages/`로 올리지 않는
 * 이유는 **두 편집기의 역할이 다르기 때문**이다 — 어드민 것은 운영진이 어떤 폼이든 짜는 폼
 * 빌더이고 이것은 스터디장이 자기 모집 폼 한 장을 손보는 화면이다. 공유 부품으로 묶으면
 * **한 앱의 화면 변경이 다른 앱의 계약이 된다.**
 *
 * ⚠️ **여기 적혀 있던 근거는 실측과 맞지 않았다.** «어드민 편집기가 라벨·복제·삭제·접수 상태·
 * 자동 저장까지 한 덩어리인데 그중 대부분이 리더에게 필요 없다»고 적혀 있었는데, 2026-09-26에
 * 재 보니 **공개 계약은 5개 공통 + 3개 갈림**이고(`questionsLocked` / `readOnly`·`onWarn`) 그
 * «한 덩어리»는 편집기가 아니라 **호출부**에 있다(아래 `onChange` 주석 그대로). 코드도 이
 * 파일의 **84.8%가 어드민과 같다**(512/604줄). 그 문장을 근거로 판단하면 잘못 판단한다.
 *
 * 중복은 대가를 치른 적이 있다 — web#698에서 `aria-expanded`가 **두 사본의 같은 자리**에
 * 빠져 있었다. **같은 일이 한 번 더 나면 ADR-0052를 다시 본다**(그 ADR의 «무엇이 바뀌면 다시
 * 본다»에 그렇게 적혀 있다).
 *
 * ── 옮기면서 갈린 것 ───────────────────────────────────────
