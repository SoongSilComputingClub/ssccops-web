...
export const ROUTES = {
  /**
   * 첫 화면 — 갈 곳을 고르는 자리 (#228).
   *
   * 전에는 루트가 `/studio`로 곧장 넘겼지만(#169), 상단 바가 역할별로 갈리고 나서(#224) 일반
   * 회원에게는 첫 화면이 "학술 대시보드" 제목 아래 빈 상태만 뜨는 꼴이 됐다. 지금은 기획안
   * 제출·제출 현황 두 카드를 그리고, **스터디장은 `/studio`로 곧장 넘긴다**(페이지의 redirect).
   * 스터디장이 걸러진 뒤라 대시보드 카드는 두지 않는다 — 눌러도 빈 상태뿐이다.
   */
  home: "/",
  /**
   * 학술 대시보드 — 스터디장의 홈.
  ...
  studio: "/studio",
  /**
   * 내 프로그램 목록 — 내가 스터디장/팀장인 프로그램들 (#188).
   *
   * 대시보드(`/studio`)가 "지금 굴러가는 활동 하나"라면 이 화면은 "내가 맡은 활동 전부"다.
   * 상단 바의 "내 프로그램"이 이 주소를 가리킨다(그전에는 `/studio`로 걸려 대시보드와 같은
   * 화면이 떴다). 여러 활동을 맡지 않은 회원에게는 빈 상태를 그린다.
   */
  studioPrograms: "/studio/programs",
  /**
   * 프로그램 하나의 상세 — 커리큘럼 대비 진행·회차 이력·출석 요약 (#188).
   *
 ...
 *
 * 경로 문자열을 화면에 직접 조립하지 않는다. 내 프로그램 목록의 카드·대시보드의 프로그램 카드가
 * 이 빌더를 쓴다.
 */
 ...
 *
 * 경로 문자열을 화면에 직접 조립하지 않는다. 내 프로그램 상세·프로그램 선택 목록이 이 빌더를 쓴다.
 */
