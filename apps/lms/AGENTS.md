...
- **목록 머리의 «이 앱 | 전체» 칩**(#643 · ADR-0047 · server#535) — 기본 «이 앱»은 `?app=LMS`(기준표가 LMS를 수신 앱으로 둔 알림만), «전체»는 파라미터 없이 회원의 모든 알림이다. 값은 훅의 상태이고 **주소에도 localStorage에도 남기지 않는다** — 다시 들어오면 «이 앱»이다. 바꾸면 커서가 앞 범위의 것이라 목록을 처음부터 다시 부른다. «전체» 응답의 `unreadCount`는 남의 앱 것까지 세므로 배지에 넣지 않고, 그 범위에서 한 건을 읽으면 `app=LMS`로 다시 묻는다. 행의 `app`이 LMS가 아니면 «운영 ↗»·«홈페이지 ↗» 꼬리표가 붙는다(`NOTIFICATION_APP_LABEL` · 공유 컴포넌트의 `currentApp`) — **오리진이 없어 못 가는 행도 꼬리표는 그대로**이고 누르면 읽음 처리만 되고 머문다. 칩·꼬리표 모두 `@ssccops/pwa/ui`라 세 앱이 같다.
