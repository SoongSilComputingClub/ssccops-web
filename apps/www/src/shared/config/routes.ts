...
export const ROUTES = {
  ...
  /**
   * 내 알림 — 낸 응답의 검토 결과·행사 참가 상태 (#616 · ssccops#453 · ADR-0045).
   *
   * 진입은 상단 바의 종뿐이고 목차(`nav-links.ts`)에는 없다 — 목차는 «할 수 있는 일»이고 알림은 그
   * 일로 가는 문이다. 로그인이 필요한 화면이라 미들웨어 매처와 `robots.ts`의 차단 목록에 함께 든다.
   */
 ...
 * 사용처였던 '내 신청'의 가입 안내가 이제 같은 자리에서 `SignupStep`을 연다. 부원에게 나가는 화면이
 * 운영 도메인을 가리키지 않는다. #616에서 같은 이름의 env가 돌아왔지만 **화면 링크가 아니라 알림**
 * (운영진 본인이 받은 어드민 알림을 눌렀을 때 · 서비스워커)에만 쓴다 — `shared/config/app-origins.ts`.
 */
