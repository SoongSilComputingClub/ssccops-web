 ...
 *
 * ── 조회 실패는 절을 통째로 감춘다 ──────────────────────────
 * 홈(#595)에서는 이 절이 여러 블록 중 하나다 — 목록을 못 받았을 때 오류 한 줄을 세우면 다른
 * 블록을 보러 온 사람이 오류를 먼저 본다(홈 배너가 실패를 «없음»으로 그리는 것과 같은 판단).
 * `/academic`에서도 아래 «참여하기»(`AcademicCta`)가 남아 길이 끊기지 않는다. 받았는데 비었으면
 * 빈 상태 한 줄(그것은 정상 상태다). 세션은 보지 않는다(익명 캐시 · `AcademicCta`와 같다).
 */
