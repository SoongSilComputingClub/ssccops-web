...
/**
 * 최근 활동 — 게시 포스트 최신순 (#524 · ssccops#385).
 *
 * 카드는 아카이브 목록과 같은 것(`entities/content` `PostCard` — 표지 · 분류 · 제목 · 요약 ·
 * 활동일)이고 «전체 보기»가 `/activities`로 간다. 조회에 실패하면 «—» 한 줄, 게시된 글이
  ...
  return (
    <section className="flex flex-col gap-[12px]">
      <div className="flex items-baseline justify-between gap-[10px]">
        <h2 className="text-[19px] font-semibold tracking-[-.2px]">최근 활동</h2>
        <Link href={ROUTES.activities} className="text-[14px] text-accent-strong">
          전체 보기 ›
        </Link>
      </div>
      {body}
