 ...
 *
 * **여러 건을 받는 폼에서만 그린다.** 1건 폼에서는 제출을 마치는 순간 화면이 '이미 제출한
 * 폼입니다'로 갈리므로 여기 설 자리가 없고, 아직 안 낸 사람에게는 언제나 빈 상자다.
 *
...
export function MyResponsesPanel({ formId }: Readonly<{ formId: number }>) {
  ...
  return (
    <Card className="flex flex-col gap-[2px]">
      ...
      {status === "loading" && (
        <div className="mt-[6px] text-[13.5px] text-n500">불러오는 중입니다…</div>
      )}
      ...
      {status === "ready" && responses.length === 0 && (
        <div className="mt-[6px] text-[13.5px] text-n500">아직 낸 응답이 없습니다.</div>
      )}

      {status === "ready" && responses.length > 0 && (
        <div className="mt-[10px] flex flex-col gap-[8px]">
          {responses.map((r) => {
            const badge = RESPONSE_STATUS_BADGE[r.rspnsSttsCd];
            return (
              <div
                key={r.formRspnsId}
