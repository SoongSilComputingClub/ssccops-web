...
/*
 * 내가 이끄는 프로그램 — 스터디·프로젝트·트랙 (SSR · #574 · ssccops#428) — 허브 `/me`의 ④를 한 장으로.
 *
 ...
 * 한 사람이 맡는 프로그램은 대개 한두 건이라 거를 것이 없고, `APPROVED`는 배지도 없다.
 */
export async function MeProgramsPage() {
  return (
    <MeFrame
      pathname={ROUTES.mePrograms}
      description="스터디장·팀장으로 맡은 프로그램의 진행 상황을 확인할 수 있습니다"
    >
      <Body />
    </MeFrame>
  );
...
function ProgramList({ programs }: Readonly<{ programs: AcademicProgramSummary[] }>) {
  if (programs.length === 0) return <EmptyState title="이끄는 프로그램이 없습니다" />;
  return (
    <div className="flex flex-col gap-[12px]">
