...
/*
 * 재제출 폼 (ssccops#221 · 클라이언트).
 *
 * ── 왜 클라이언트인가 ──────────────────────────────────────
 * 이 앱은 조회를 서버 컴포넌트로 그리지만, 답을 고쳐 가며 제출하는 부분은 서버 렌더만으로
 * 그릴 수 없다. 상세가 도착한 뒤에야 이 컴포넌트를 마운트하므로 `useState` 초깃값이 곧 폼
 * 초깃값이다 — 동기화용 `useEffect`가 없다.
 *
 * ── 프리필 ────────────────────────────────────────────────
 * 서버가 준 이전 답(`rspnsCn`)을 그대로 초깃값으로 넣는다. 재제출은 **전체 본문 재전송**이고
 * 임시저장이 없으므로(`ssccops-server#177` 결정 2), 사용자가 고친 값을 들고 있다가 제출 한
 * 번으로 보낸다.
 *
 * ── 검증은 `@ssccops/form-renderer` ─────────────────────────
 * 페이지 이동('다음')·제출에서 패키지 함수를 부른다. 필수·정규식·최대 선택 수·분기를 화면에서
 * 다시 판정하지 않는다(AGENTS.md).
 *
 * ── 문항 설명 ─────────────────────────────────────────────
    ...
    <div className="flex flex-col gap-[12px]">
      ...
      {pageQitems.map((qitem) => (
        <QitemCard
          key={qitem.qitemId}
          qitem={qitem}
          value={form.answers[qitem.qitemId]}
          error={form.errors[qitem.qitemId]}
          onChange={(value) => form.setAnswer(qitem.qitemId, value)}
        />
      ))}
