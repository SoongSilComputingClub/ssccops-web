 ...
 *
 * 1차에서는 설치 유도(프롬프트·배너)와 서비스워커·푸시가 범위 밖이었다. 이 앱의 방문자 대부분은
 * 카톡·에타 링크로 한 번 들어와 행사를 보고 나가므로 설치를 권할 대상이 뚜렷하지 않다 — 대신
 * 재학생이 스스로 홈 화면에 추가했을 때 아이콘 자리에 페이지 스크린샷이 박히는 일은 막는다.
 * 2차(#607 · ssccops#449 · ADR-0045)가 서비스워커(`app/sw.js/route.ts`)·오프라인 안내(`app/offline`)·
 * 재방문자 설치 띠(`features/pwa`)를 얹었다 — 첫 방문에 띄우지 않는 것은 #150의 판단 그대로다.
 * 푸시는 여전히 없다. 이 파일은 그대로다.
 *
 * ── start_url이 "/"인 이유 ────────────────────────────────
