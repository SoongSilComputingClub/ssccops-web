...
/**
 * 종 — 안 읽은 알림 수 배지 + `/notifications` 링크 (#616 · ssccops#453 · admin #604·lms #606과 같은 모양).
 *
 * 값은 `@ssccops/pwa`의 스토어에서만 읽는다(듣는 것은 `UnreadCountSync` 하나). 아직 못 들었거나 0이면
 * 배지가 없다. 드롭다운을 두지 않고 페이지로 보내는 것은 모바일에서 읽음 처리·더 보기가 드롭다운에
 * 비좁아서다. 크기·테두리는 옆의 드로어 버튼(`mobile-nav.tsx` ☰)과 같다. 자리는 `UtilityCluster`의
 * `bell` 슬롯(#614) — 로그인한 사람에게만 있고, 상단 바 하나가 데스크톱·모바일을 함께 맡는다.
 */
