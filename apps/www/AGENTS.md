...
**부원과 외부인이 보는 앱이다.** 홈(#524)·행사 목록·상세와 콘텐츠(소개·운영진·기록 아카이브·모집·문의·법적 페이지 · #520 · ADR-0038)는 로그인 없이 열리고(wave2 D1 — 비로그인 완전 공개), 신청·내 활동(`/me`)·공개 폼 응답만 로그인이 필요하다. 이 앱이 아는 남의 오리진은 **lms 하나**(`shared/config/lms-routes.ts` — 학술 공유 착지가 사람을 보낼 곳)다. 어드민 오리진(`NEXT_PUBLIC_ADMIN_ORIGIN`)은 #451에서 걷어냈다 — 부원에게 나가는 화면이 운영 도메인을 가리키지 않는다.
...
| `/academic` | **학술**(SSR · #550 · ssccops#412) — 페이지 `academic`을 `ContentPage`로 그리고 아래에 `AcademicCta`(«LMS로 가기» · «기획안 제출하기» — `NEXT_PUBLIC_LMS_ORIGIN`이 없으면 블록 없음). 상단 바 «학술»이 여기로 온다 — 다른 앱으로 바로 나가지 않고 «학술 프로그램이 어떻게 돌아가는가»를 읽는 자리. 세션을 보지 않는다(로그인은 LMS가). 본문과 CTA 사이에 **«모집 중인 학술 프로그램»**(`AcademicPrograms` · #587 · ssccops#435 · ADR-0043) — `/events`가 뺀 `academicProgram` 있는 행사를 **리스트형**(#591 · ssccops#439 — 유형 · 제목 · 모집 · 기간, `shared/ui/list-table.tsx` 틀 · lg 미만은 카드로 접힘 · 상세는 기존 행사 상세)으로. #587의 유형별 카드 격자는 학기 초 스터디 열 건이 한 화면을 다 먹어 걷었다. 접수 기간·정원·확정은 목록 계약에 없어 열이 없다(기간 열은 행사 기간). 비면 «지금 모집 중인 학술 프로그램이 없습니다» 한 줄, 목록 조회 실패면 절을 감춘다(본문이 본체다). 학술 도메인에 공개 목록 API를 따로 열지 않았다(신청이 행사 신청 화면이라 두 축이 같은 건) | 익명 |
...
- **상단 바는 일곱 항목**(SSCC · 운영진 · 기록 · 행사 · 학술 · 모집 · 문의 · `app/_shell/nav-links.ts`)이고 **«지원하기» CTA는 없다**(ssccops#382) — 모집 때는 홈 배너(페이지 `home-banner` · #524)가 안내한다. «행사»는 #520에서 «활동» 축이 품었는데 행사 목록으로 가는 길이 홈의 «행사 전체 보기» 하나뿐이라 #529에서 항목으로 세웠다(`/events`·`/events/{id}`에서 «행사»가 켜진다). «문의»는 `/contact` 화면이다(#524 — #520에서는 푸터 블록 `#contact`로 가는 앵커였는데 켜지지도 않고 공유할 주소도 없었다). 푸터의 문의 블록은 그대로 두고 두 자리가 같은 `shared/config/contact.ts`를 읽는다 — **메일은 옛 사이트에도 없어 null**이고 정해지면 거기 한 줄. 404 화면은 홈·기록 두 링크(#524).
...
- **다른 앱으로 가는 항목은 `externalNavLinks()`다**(#577 · ssccops#430 · `app/_shell/nav-links.ts`). 지금은 «LMS ↗» 하나 — `NEXT_PUBLIC_LMS_ORIGIN`이 비면 항목이 없다(죽은 주소 금지). `NAV_LINKS`에 섞지 않는 것은 저쪽이 이 앱의 화면이라 `isActive`가 있고 `<Link>`로 가기 때문이며, 외부 앱은 `<a>`로 나간다. 데스크톱·드로어·푸터 셋이 같은 함수를 읽는다. «학술» 랜딩(#550)의 CTA는 설명 뒤의 유도, 이것은 이미 LMS를 쓰는 부원의 지름길이다.
