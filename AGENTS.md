...
- **대시로 다음 행동을 잇는 문장은 마침표를 찍지 않고, 상태만 알리는 문장은 찍는다** — 제목
  (`title`·`EmptyState`의 첫 줄)에는 어느 쪽이든 찍지 않는다. 새로 만든 규칙이 아니라 코드에
  이미 서 있던 것을 적어 둔 것이다(#343 · lms 전수 확인). 대시 문장에 마침표를 붙이면 "다음
  행동"이 문장 끝에 매달린 부연으로 읽혀 두 성격의 문장이 뒤섞인다.
- **보조용언은 붙여 쓴다** — `시도해주세요`·`로그인해주세요`·`확인해주세요`(`시도해 주세요`가
  아니다). 국립국어원이 둘 다 허용해 한 낱말이 화면마다 다르게 적히던 것을 한쪽으로 못 박은
