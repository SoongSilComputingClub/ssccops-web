...
'내 신청'(`/my-applications`)이다. wave2 결정 D1(비로그인 완전 공개) · D7(SSR/OG) ·
D12(md 안전 렌더링) · D10(능동 통보 없이 '내 신청'에서 결과 확인)을 그대로 구현한 것이라,
아래는 취향이 아니라 결정 사항이다.

- **본체는 여전히 익명이고, 로그인은 '내 신청' 한 화면에만 붙어 있다**(#150). 목록·상세는
  토큰 없이 그리고, `shared/api/client.ts`는 **봉투 벗기기와 `ApiError`만** 남긴 익명 전용
  구현 그대로다. 인증 호출은 옆에 **더한** `shared/api/authed-client.ts`가 맡는다 —
  Authorization 헤더를 싣는 것만 다르고 봉투·오류 변환은 `apiFetch`를 그대로 통과한다.
- **인증 실패에 리다이렉트를 걸지 않는다.** 이 앱에는 로그인 화면도 가입 폼도 없다. 401은
  '내 신청' 화면 안의 로그인 유도로, 403 `SIGNUP_REQUIRED`는 **가입 안내**(어드민 오리진이
  설정돼 있으면 링크, 없으면 문구만)로 그린다 — 어드민의 `apiFetch`처럼 리다이렉트를 옮겨
  오면 되돌아올 곳이 없어 왕복만 도는 길이 생긴다.
- **미들웨어 매처는 `/my-applications` 하나다.** 하는 일도 세션 쿠키 갱신뿐이고 가드가
  아니다(`shared/lib/supabase/proxy.ts`). 매처를 넓히면 로그인하지 않는 방문자의 조회마다
  Supabase 왕복이 붙는다.
- **전 화면이 서버 컴포넌트다** — 예외는 로그인 상태를 쥐어야 하는 `features/auth`의 두
  컴포넌트뿐이다(`AuthNav`·`SignInButton`). 상세를 SSR로 그리는 이유가 OG 메타태그이고
...
- **신청 제출 흐름(신청서 작성·제출)은 아직 없다.** 상세의 신청 버튼은 여전히 **잠긴 채로 서
  있고** 준비 중임을 안내한다 — 감추면 신청 방법이 없는 공지형 행사와 구별되지 않는다.
  '내 신청'은 이미 낸 신청을 **보여 주기만** 한다.
