...
export function PublicFormDonePage({ slug }: { slug: string }) {
    ...
    <div className="flex min-h-screen items-center justify-center px-6">
      <div className="w-[520px] rounded-2xl bg-surface p-8 text-center shadow-[0_0_0_1px_#e5e8eb]">
        ...
        <div className="mt-2 text-[14.5px] leading-[1.6] text-n400">
          {form ? `${form.title} 응답이 접수됐어요. ` : ""}결과는 등록한 연락처로
          안내드려요.
        </div>
        ...
        <div className="flex gap-2">
          <button
            type="button"
            onClick={() => router.push(ROUTES.publicForm(slug))}
          ...
          <button
            type="button"
            onClick={() =>
              router.push(form ? ROUTES.formDetail(form.key) : ROUTES.dashboard)
            }
