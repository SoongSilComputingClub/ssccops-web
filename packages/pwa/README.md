...
| `serviceWorkerResponse({ app, appOrigins, cacheVersion?, apiBaseUrl?, deployEnv?, offlinePath? })` | `GET /sw.js`의 **응답 한 벌**(#671) — `buildServiceWorker`를 부르고 헤더 셋을 붙여 표준 `Response`를 돌려준다. `apiBaseUrl`에서 오리진을 뽑고 `deployEnv`로 알림 아이콘(`deployMarks(...).mark`)을 고르는 것까지 여기 있다. 앱의 라우트 핸들러에 남는 것은 `export const dynamic = "force-dynamic"`과 **env를 글자 그대로 읽는 일**뿐이다 — `NEXT_PUBLIC_*`은 빌드 때 치환되므로 앱 파일에 적혀 있어야 한다. `offlinePath` 기본값 `/offline` |
...
| `createPushApi(apiFetch, apiFetchNullable?)` | `/v1/push/*` 구독 호출 한 벌(`config`·`subscribe`·`unsubscribe`) — 세 앱에 한 벌씩 있던 사본을 #671에서 올렸다. `createNotificationApi`와 같은 자리로 **보내는 통로만 받는다**. `DELETE`가 `data` 없는 200 봉투라 그것을 오류로 세우지 않는 통로가 따로 있는 앱(www·lms)은 둘째 인자로 넘기고, 없는 앱(admin)은 하나만 넘긴다. 서버가 모르는 endpoint의 404는 성공으로 삼킨다(앱의 `ApiError`를 모르므로 `status` 필드만 duck-typing) |
...
| `useUnreadCountSync({ api, app, setUnread? })` | 종 배지 값을 **듣는** 훅(#671) — 진입 때 한 번, 탭이 다시 보일 때마다 `GET /v1/notifications/unread-count?app=<이 앱>`. 세 앱에 글자까지 같은 사본이 있었다. 실패는 조용하다(배지를 안 그릴 뿐). **한 앱에서 한 곳만 부른다** — 로그인했다고 판정된 뒤의 자리에 빈 컴포넌트(`UnreadCountSync`) 하나. `setUnread` 기본은 이 패키지의 모듈 스토어이고 admin이 자기 zustand setter를 꽂는다 |
| `useGatedNotificationList({ api, app, resolveTarget, toErrorMessage, isUnauthenticated, isSignupRequired })` | **401·403을 화면 상태로 올리는 앱**(www·lms)의 `useNotificationList` 배선(#671) — `useRouter().push`와 `classifyStatus`(401 → `unauthenticated` · 403 `SIGNUP_REQUIRED` → `signup-required`)를 여기서 만든다. 판정 두 함수는 그 앱의 `ApiError`를 아는 `shared/api/auth-error.ts`에서 받는다. admin은 쓰지 않는다 — `apiFetch`가 401·403을 리다이렉트로 끝내 상태가 셋뿐이다 |
...
| `@ssccops/pwa/ui` → `NotificationScreen({ list, settings, children, gate?, sectionClassName? })` · `NotificationPageHeader({ description, action? })` | `/notifications` 화면의 몸통과 머리(#671) — «알림 설정» 절(`hidden` 마운트)과 훅의 상태를 `NotificationList`에 잇는 배선이 세 앱에 같았다. 슬롯은 셋: 설정 카드는 `children`(앱이 `onStateChange={settings.onPushStateChange}`를 꽂아 넘긴다), `gate`는 로그인·가입 안내(www·lms만 — 서 있으면 목록도 설정 절도 그리지 않는다), `sectionClassName`은 부모가 `flex gap`이 아닌 앱(admin `mb-5`)의 여백. «모두 읽음» 실패 한 줄은 훅의 `actionError`이고 토스트를 꽂은 앱에서는 언제나 비어 있다. 머리는 www·lms만 쓴다 — admin은 자기 `PageHeader`·`PageBody` |
| `@ssccops/pwa/ui` → `NotificationBell({ href })` | 종 — 안 읽은 수 배지 + 알림 목록 링크(#671). 값은 이 패키지의 모듈 스토어에서만 읽고 갈리는 것은 그 앱의 `ROUTES.notifications`뿐이다. www·lms가 쓴다 — admin은 종이 두 자리(상단 바 md · 사이드바 sm)이고 값도 자기 zustand라 아직 사본이다 |
...
2. `app/sw.js/route.ts` — `serviceWorkerResponse({ app, appOrigins, ... })` 한 줄 + `export const dynamic = "force-dynamic"`(env는 이 파일이 글자 그대로 읽는다). `app/offline/page.tsx` — 정적, 데이터 없음. **둘 다 미들웨어 매처에서 뺀다** — 로그인 전에 `/sw.js`가 `/login`으로 리다이렉트되면 등록 자체가 실패하고(워커 스크립트는 리다이렉트를 못 따라간다), install이 `/offline`을 담을 때 로그인 HTML을 담는다.
...
5. `entities/push`는 `createPushApi(앱의 apiFetch[, nullable 통로])` 한 줄이다(#671) — `DELETE /v1/push/subscriptions`가 404(모르는 endpoint)면 성공으로 삼키는 것은 패키지가 든다. 브라우저 구독은 어느 쪽이든 풀어야 한다.
6. `entities/notification`은 `createNotificationApi(앱의 apiFetch)`와 **`CURRENT_APP`**(이 앱의 `PushApp`)만 두고, 목록·안 읽은 수 조회에 그 값을 `app`으로 싣는다(#643 · ADR-0047 · server#535). `/notifications` 화면 상태는 `useNotificationList`에 그 둘과 이동 규칙을 꽂아 받는다(#665 — 아래 규칙은 그 훅이 지킨다). **종 배지는 언제나 «이 앱» 수다** — «전체» 칩에서는 `app`을 빼고 목록을 부르되 그 응답의 `unreadCount`는 스토어에 넣지 않고, 그 범위에서 한 건을 읽으면 하나 빼는 대신 `app=<이 앱>`으로 다시 묻는다(읽은 행이 이 앱에도 오는 알림인지는 기준표가 안다). 범위를 바꾸면 커서가 앞 범위의 것이라 목록을 처음부터 다시 부른다. 401·403을 화면 상태로 올리는 앱(www·lms)은 그 위의 배선까지 `useGatedNotificationList`가 든다(#671). 배지를 듣는 자리는 `useUnreadCountSync`를 부르는 빈 컴포넌트 하나이고, 화면은 `NotificationScreen`(+ www·lms는 `NotificationPageHeader`·`NotificationBell`)에 값을 꽂는다.
