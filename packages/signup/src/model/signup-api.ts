import type { AuthedApiFetch } from "./api-failure";
import type { MemberProfileResponse, SignedUpMember } from "./types";

/*
 * 간편 가입 (POST /v1/members/signup · ssccops-server #21).
 *
 * www·lms의 `entities/member/api/signup.ts` 두 벌에서 올렸다(#664) — 글자까지 같은 사본이었다.
 * **세션 조회와는 나눠 둔다.** 저쪽은 "이 계정이 우리 서비스의 누구인가"를 서버 컴포넌트에서
 * 읽는 조회이고, 이 요청은 브라우저에서 회원을 **만든다**(가입 화면이 신청 흐름 안에 임베드돼
 * 있다 — wave2 §8-4). 앱에서 한 배럴에 함께 두면 서버 전용 모듈(`next/headers`를 타는 세션
 * 조회)이 클라이언트 번들로 끌려 들어와 빌드가 깨진다.
 */
 ...
 *
 * 이메일·인증 계정 식별자·등급(TEMP)·전산 가입일은 **서버가 토큰과 현재 시각으로 채우므로 여기에
...
export async function signUp(
  ...
  const member = await apiFetch<MemberProfileResponse>("/v1/members/signup", {
    method: "POST",
