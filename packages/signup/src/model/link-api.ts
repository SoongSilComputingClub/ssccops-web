 ...
 *
 * www·lms의 `entities/member/api/link.ts` 두 벌에서 올렸다(#664).
 *
 * ── 왜 어드민으로 넘기지 않는가 ────────────────────────────────
 * 어드민에도 같은 호출이 있지만(`apps/admin/src/entities/session/api/link.ts`) 그쪽으로 넘기는
 * 것이 #364가 없앤 왕복이다. 명부에 있는 사람이 신청하러 와서 학번 중복으로 막히면 어드민에서
 * 다시 로그인하고 연결한 뒤 신청서로 돌아와야 했다 — 같은 화면이 "다른 화면으로 이동하지
 * 않습니다"라고 적어 두고 그 사람만 예외로 두던 자리다.
 *
 ...
 * 항목별 오류를 그리지 않는다(`model/link-form.ts`).
 */
...
export async function linkExistingMember(
  ...
  const member = await apiFetch<MemberProfileResponse>("/v1/members/link", {
    method: "POST",
