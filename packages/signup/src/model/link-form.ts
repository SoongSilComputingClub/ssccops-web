...
import { MEMBER_LINK_ERROR, type MemberLinkRequest } from "./link-api";
 ...
 *
 * ── 어드민은 왜 이 패키지를 쓰지 않는가 ─────────────────────
 * www·lms는 사본이 글자까지 같아 여기로 올렸지만(#664), 어드민은 **감싸고 있는 것이 전부
 * 다르다.** 어드민의 `apiFetch`는 401·403을 리다이렉트로 끝내고(공개 앱은 밀어낼 로그인 화면이
 * 없어 오류로 올린다), 성공 뒤 가는 곳도 다르며(가입 완료 화면 ↔ 지금 이 자리의 신청서), 오류를
 * 그리는 부품도 다르다 — `Button`·토스트가 앱마다 다르다는 것은 ssccops-web#322·#335에서 두 번
 * 확인됐다. **판단이 갈리지 않도록 근거를 양쪽 주석에 서로 가리켜 둔다.**
 */
...
export function toMemberLinkFailure(error: unknown): MemberLinkFailure {
  ...
  switch (failure.code) {
    ...
    case MEMBER_LINK_ERROR.TOO_MANY_LINK_ATTEMPTS:
      return {
        kind: "locked",
        message: failure.message || "연결 시도가 너무 많습니다 — 잠시 후 다시 시도해주세요",
      };

    case CLIENT_ERROR.UNAUTHENTICATED:
    case CLIENT_ERROR.UNAUTHORIZED:
      return {
        kind: "form",
    ...
    case CLIENT_ERROR.CONFIG_MISSING:
      return {
        kind: "form",
        message: "지금은 연결할 수 없습니다 — 잠시 후 다시 시도해주세요",
      };
