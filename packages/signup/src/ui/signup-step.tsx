...
import { signUp, type SignupStatusCode } from "../model/signup-api";
import {
  EMPTY_SIGNUP_VALUES,
  buildSignupRequest,
  hasErrors,
...
/*
 * 간편 가입 — **가입하려는 사람이 선 자리에서 끝낸다** (wave2 §8-4 · www #451 · lms #453).
 *
 * 화면을 옮기지 않는 것이 이 컴포넌트의 요점이다. 가입 화면으로 리다이렉트하면 돌아올 곳을
 * 쿼리로 나르고, 돌아온 뒤 다시 하려던 것을 불러오는 왕복이 두 번 더 생긴다 — 4단계 중 이탈이
 * 가장 큰 구간이 바로 여기다. 가입이 끝나면 `onSignedUp`으로 알리고, 부모가 같은 자리에서
 * 다음 단계로 넘긴다(www는 신청서, lms는 `router.refresh()`).
 *
 * 등급은 요청에 없다 — **서버가 임시회원(TEMP)으로 고정한다.** 클라이언트가 보낸 등급을 믿고
 * 저장하면 스스로 등급을 올릴 수 있다.
 *
 * **이 컴포넌트는 앱을 모른다.** 행사도 학술 프로그램도 모르고, 아는 것은 «회원이 되었다»는
 * 사실 하나뿐이다 — 그래서 www의 신청 흐름·`/me`·공개 폼과 lms의 미가입 안내가 같은 화면을
 * 쓴다(#664). 앱마다 다른 것은 인증 호출(`apiFetch`) 하나다.
 */
export interface SignupStepProps {
  /**
   * 이 앱의 인증 API 호출 — 봉투를 벗겨 `data`만 돌려주는 브라우저 통로.
   *
   * www·lms 모두 `shared/api/browser-client.ts`의 `apiFetchAuthedFromBrowser`를 넘긴다. 패키지가
   * 전송 계층을 직접 들지 않는 이유는 `model/api-failure.ts` 머리 주석에 있다.
   */
...
export function SignupStep({
  ...
  const submit = async () => {
    ...
    try {
      await signUp(apiFetch, buildSignupRequest(values, statusCode));
