 ...
 * 오프셋·D-day·마감 배지, www의 행사 기간 표기. **lms에는 이제 고유한 것이 없다**(#340에서
 * 점 표기를 지웠다 — 아래 `formatYmd` 주석).
 *
 ...
 * 화면은 없다. lms에 빈 문자열을 받아 점 표기로 떨어뜨리던 자리(`formatDt(x) || formatYmdDotted(x)`)가
 * 둘 있었는데, **같은 칸이 데이터에 따라 모양을 바꾸는 자리**라 #340에서 걷어냈다 — 승인 일시는
 * 일시TS이므로 `formatDt` 하나로 두고, 값이 없으면 빈 자리로 둔다.
 */
 ...
 * 취향**이라 근거의 종류가 달라, #328은 그 앱에 `formatYmdDotted`라는 제 이름으로 남기고
 * 표기를 통일할지는 화면을 보고 정하기로 했다.
 *
 * **#340이 그 확인을 수행했고 통일로 결론냈다** — 한 화면 안에서 `계획일 2026. 3. 1.`과
 * 제출 일시 `2026-08-28 19:00`이 나란히 서고 있었다. 폭이 좁은 자리도 점 표기 쪽이 오히려
 * 넓어(`2026. 12. 31.` > `2026-12-31`) 표가 깨질 걱정도 반대였다. 이제 **세 앱이 모두 이
 * 함수를 쓴다.**
 *
