...
/**
 * 화면 테마 (ssccops#226 · ssccops-web#341).
 *
 * **admin이 #226에서 정한 구조를 그대로 공유로 올린 것이다.** lms에 다크 모드를 넣으며(#341)
 * 두 앱이 같은 것을 쓰게 됐다 — 사본을 두면 저장 키·3상태 판정·FOUC 스크립트가 갈릴 수 있고,
 * 갈린 것을 타입도 린트도 잡지 못한다(`cn`·`@ssccops/date`가 걷어 온 것과 같은 자리다).
 *
 * 저장 키(`sscc-theme`)를 앱마다 나누지 않는 것은 **오리진이 이미 다르기** 때문이다 —
 * admin·lms는 서로 다른 도메인이라 localStorage가 처음부터 분리돼 있다. 같은 이름을 쓰는 것은
 * 한 사람이 두 앱에서 같은 취향을 고르리라는 뜻이 아니라, 키를 지어낼 이유가 없다는 뜻이다.
 *
 ...
 * **토글이 두 곳에 그려진다** — admin은 NavPanel 마크업 한 벌을 사이드바와 모바일 드로어가
 * 함께 쓰고, lms는 상단 바와 모바일 드로어에 **각각** 그린다(#341 — 셸 구조가 갈려 있다).
