 ...
 *
 * 이미지·링크 표식의 정규식은 `[`가 닫히지 않은 채 길게 이어지면 길이의 제곱으로 되돌아간다
 * (#401 · S8786). 그대로 두는 것은 입력이 **운영진이 어드민에서 적은 본문**(행사 본문·폼
 * 안내)이지 익명 방문자의 값이 아니어서다 — 여기까지 온 사람은 본문에 무엇이든 실을 수 있는
 * 권한을 이미 가졌고, 표식을 이보다 정확히 가르려면 파서를 들여야 해 이 자리의 목적을 넘는다.
 */
